...
from vs_time_graphs import plot_intensity_vs_time, plot_all_peaks_combined
...
if __name__ == "__main__":
    ...
    zoom_all_raw = input(
        "\n Do you want to replot all RAW spectra with custom zoom limits? (y/n): "
    ).lower()
    ...
    zoom_all_subtracted = input(
        "\n Do you want to replot all SUBTRACTED spectra with custom zoom limits? (y/n): "
    ).lower()
    ...
    print(
        f"The extracted max peaks for reactants was successfully saved in {dirs['csv_reacts']}"
    )
    ...
    print(
        f"The extracted max peaks for products was successfully saved in {dirs['csv_prods']}"
    )
    ...
    plot_intensity_vs_time(
        dirs["csv_reacts"], clr.max_colours_react, dirs["data_proc_plots"]
    )
    plot_intensity_vs_time(
        dirs["csv_prods"], clr.max_colours_prod, dirs["data_proc_plots"]
    )
    # Plots the extrcated peak intensities over time for both reacts and prods on the same plot
    plot_all_peaks_combined(
        dirs["csv_reacts"],
        clr.max_colours_react,
        dirs["data_proc_plots"],
        title="Combined Peak Intensities - Reactants",
    )
    plot_all_peaks_combined(
        dirs["csv_prods"],
        clr.max_colours_prod,
        dirs["data_proc_plots"],
        title="Combined Peak Intensities - Products",
    )
