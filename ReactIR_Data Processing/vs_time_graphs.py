...
def add_trendline_single(x_vals, y_vals, color, alpha=0.6, linestyle='--'):
    ...
    plt.annotate(
        ...
        ha='center',
        fontsize=9,
        bbox=dict(boxstyle="round,pad=0.4", fc=pale_color, ec=color, lw=1.2, alpha=0.9)
    )
...
def add_trendline_multi(x_vals, y_vals, color, alpha=0.6, linestyle='--'):
    ...
    line, = plt.plot(x_vals, y_pred, linestyle=linestyle, color=color, alpha=alpha, label=label)
    return line

#-----------------------------------------------------------------------------------------------------------

# This has the same logic as the above funciton plot_intensity_vs_time() but uses a different colour index
def plot_all_peaks_combined(folder_path, color_list, data_proc_plot_dir, title):
    colour_index = 0
    ...
    plt.tight_layout(rect=[0, 0, 0.75, 1])  # 0.75 = reserves 25% of width for legend
