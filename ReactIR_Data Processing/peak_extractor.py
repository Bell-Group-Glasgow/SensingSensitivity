...
def extract_top_peaks(wavenumbers, intensity_series, num_peaks=6, exclusion_window=10):
    ...
    df_sorted = df_filtered.sort_values(by='Intensity', ascending=False)

    selected_peaks = []
    
    for _, row in df_sorted.iterrows():
        candidate = row['Wavenumbercm-1']
        
        # Checks to see if the candidate peak is at least `exclusion_window` away from all previously selected peaks
        if all(abs(candidate - selected) > exclusion_window for selected in selected_peaks):
