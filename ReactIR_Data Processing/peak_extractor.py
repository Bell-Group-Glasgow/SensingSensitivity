...
def extract_top_peaks(
    wavenumbers, intensity_series, num_peaks=6, exclusion_window=10
):

    # Creates a temporary DataFrame for wavenumbers and intensities
    df_temp = pd.DataFrame(
        {"Wavenumbercm-1": wavenumbers, "Intensity": intensity_series}
    )
    ...
    for _, row in df_sorted.iterrows():
        ...
        if all(
            abs(candidate - selected) > exclusion_window
