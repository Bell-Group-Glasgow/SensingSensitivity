...
def setup_directories(base_path):
    ...
    dirs = {
        ...
        "input_csvs": os.path.join(base_path, "CSV"),
        ...
        "solv_minus_plot": os.path.join(
            base_path, "Results", "plots", "Solvent Subtracted Spectra"
        ),
        "data_proc_plots": os.path.join(
            base_path, "Results", "plots", "Data Processing Plots"
        ),
    }
...
def raw_csv_file(base_path):
    ...
    raw_csv_files = [
        file for file in raw_files if file.lower().endswith(".csv")
    ]
