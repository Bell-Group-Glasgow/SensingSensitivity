...
class PUMP:
    ...
    # Plunger command
    SET_SYRINGE_VOLUME = '1'            # Syringe Volume (mL). Currently only have 1mL
    ...
    def __init__(self, port: str, config: int = 6, address = 1, SPM: bool = 0, waste_port: int = 12, mode: int = 0): 
    ...
    def home(self):
        ...
        print('Valve homing complete')  #valve initialisation
        if self.SPM:
            self.checkStatus_SPM()
            print('Pump homing complete') #pump initialisation -- aka plunger being reset to starting position (0/3000 - pushed all the way in)
   
        
    def switch(self, position: str, direction='ANY', force=False):
        if direction not in ['ANY', 'CW', 'CCW']: # [shortest route, clockwise, counterclockwise]
    ...
    def move(self, position: str, movement='ABS', speed=150): #Add the speed parameter here. Default 150. Speed must be between 0 and 1600
         if movement not in ['ABS', 'DIS', 'PIC']: 
             raise ValueError('Direction parameter must be one of: "ABS","DIS","PIC"]')
         
         self.speed = speed 
         
         if self.speed > 1600: #self.speed units are pulses/s
            raise ValueError('Speed too high')
         
         self._bare_send_and_receive(self.SET_PEAK_SPEED, self.speed)
         ...
         # absolute position measured between 0 and 3000 'syrringe increments'. 0 is at the top.
         self._send_and_receive_SPM(eval(f'self.{cmd}'), position) 


    def getValvePosition(self):
        return int(self._bare_send_and_receive(self.GET_VALVE_POSITION)) #get valve position from device


    def _send_and_receive(self, command: str, parameter: str = None) -> str: #send commands to the pump and recieve information back from pump
...
if __name__=='__main__':
    ...
    # Examples of using the pump class
    
    LSP = PUMP('COM5', 12, 1, 1) # Connect to an SPM on port COM5, with a 12 port valve, gives address 1 to the SPM and home the SPM
    LSP.move(1500, 'PIC')
    LSP.switch(8, 'CCW')