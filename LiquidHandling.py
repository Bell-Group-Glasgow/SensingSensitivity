...
class Pump(PUMP):
    ...
    def transfer(
        ...
        to_position=int,
        volume=float,
        dispense_speed: int = 0.5,
        aspirate_speed: int = 0.05,
    ):
        ...
        SyringeCapacity = int(
            self.SET_SYRINGE_VOLUME
        )  # Can change if syringes are swapped.
        FullSyringes = math.floor(
            volume / SyringeCapacity
        )  # Calculates number of full aspirations required, floor(x) returns largest integer not greater than x.
        PartialSyringes = float(
            volume % SyringeCapacity
        )  # Calculates if partial aspiration is required. i.e. < 1mL for 1mL syringe.
        ...
        while FullSyringes > 0:  # Transfers however many full syringes are required.
            ...
            self.move(
                SyringeCapacity, aspirate_speed
            )  # Picks up liquid at from_position.
